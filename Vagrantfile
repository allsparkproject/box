...
$public_dns_suffix = "allspark.svc.cluster.local"
...
Vagrant.configure("2") do |config|
  ...
  #config.vm.network "private_network", ip: "172.17.8.150"
  #config.vm.synced_folder ".", "/home/core/share", id: "core", :nfs => true,  :mount_options   => ['nolock,vers=3,udp']
  config.vm.provision :shell, inline: "mkdir -p /opt/bin /opt/cni/bin", :privileged => true
  config.vm.provision "file", source: "rootfs/tmp/kubelet", destination: "/tmp/kubelet"
  config.vm.provision "file", source: "rootfs/tmp/kubeadm", destination: "/tmp/kubeadm"
  config.vm.provision "file", source: "rootfs/tmp/cni-plugins-amd64.tgz", destination: "/tmp/cni-plugins-amd64.tgz"
  config.vm.provision :shell, inline: "mv /tmp/{kubeadm,kubelet} /opt/bin/", :privileged => true
  config.vm.provision :shell, inline: "cat /tmp/cni-plugins-amd64.tgz | tar -C /opt/cni/bin -xz && rm -f /tmp/cni-plugins-amd64.tgz", :privileged => true
  ...
  (1..$num_instances).each do |i|
    ...
    #config.vm.define vm_name = "%s" % [$public_dns_suffix] do |config|
    config.vm.define vm_name = "%s-%02d.%s" % [$instance_name_prefix, i, $public_dns_suffix] do |config|
